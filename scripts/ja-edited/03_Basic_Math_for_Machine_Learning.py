...
#
# また、パラメータ $w$ と $b$ によって特徴づけられた直線の式  $(2.1)$ のような、パラメータを使って何らかの計算を行うことで、与えられたデータの特徴や関係性を表すものを、以降簡単に**モデル (model)** と呼ぶことにします（[注釈1](#note1)）。
#
...
# この関数は、予測値と目標値がすべての $n$ において完全に一致するときだけ $0$ になり、それ以外では必ず正の値をとります（[注釈2](#note2)）。
...
# なぜなら、「入力をどちらに動かせば出力が小さな値になりそうか」ということを知るためには、対象の関数を微分する必要があるからです。詳細は次章で説明します。
#
...
#
# この 3 つについて、機械学習の学習に必要となる最低限の知識に絞って以降の 3 章で解説を行います。
#
# それでは、次章はまず微分についてです。
#
# <hr />
...
# **注釈 1**
#
# 「モデル」という言葉が何を表すべきかについては、様々な考え方があり、機械学習の分野だけに限ってみても、完全に統一的な意味で用いられている訳ではありません。
#
# [▲上へ戻る](#ref_note1)
# </div>
#
